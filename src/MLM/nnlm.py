...
if __name__ == '__main__':
    ...
    if args.train_file and args.valid_file: #training mode
        ...
        for df in lm.predict(testset,bs,device=0):
            ...
            masked = df ['masked'][idx]
            ref_prob = df['ref_prob'][idx]
            ref_form = df['token'][idx]
            #pred_next = df['pred'][idx]
            #print("df.shape: ", df.shape)
            #print("idx: ",idx)
            #print(df)
            if tab_form[c] != ref_form:
                print("tab files not match predicition order: ",prefix[c], tab_form[c],ref_form)
            if not masked:
                print("masked idx error: ", prefix[c], tab_form[c],ref_form)
            #ref_form.append(ref_form)
            ref_probs.append(ref_prob)
            c += 1
        tab["probs"] = ref_probs
        #tab["pred_next"] = pred_nexts
