...
class LanguageModel(nn.Module):
    ...
    def forward(self, input, bptt_state=None,raw_out=False,mask=None):
        if isinstance(self.context_model, IncrementalTransformerContextModel):
            return self.context_model.forward(input,raw_out=raw_out,mask=mask)
        else:
    ...
    def predict(self,datagenerator,batch_size,device='cuda',use_predefined_masks=False):
        """
        ...
        for xinput, youtput, _, masks in datagenerator.generate_batch(batch_size,keep_order=True,include_mask=True):
            if not use_predefined_masks:
                masks = None
            with torch.no_grad():
                X = torch.LongTensor(xinput).to(device)   # (seq,batch,emb)
                ...
                if isinstance(self.context_model,RnnContextModel):
                   ...
                   Yhat = F.log_softmax(self.forward(X,mask=masks),dim=2)
...
def train_multi(gpu,num_gpu,model,trainset,validset,batch_size,chunk_size,epochs,warmup_epochs,warmup_batch_size,warmup_cycles,init_seed,lr,grad_clip,modeldir):
        ...
        if isinstance(model.context_model, IncrementalTransformerContextModel):
            ...
            #                             warmup_cycles)
            optimizer = optim2.Lamb(model.parameters(), lr=lr)
...
class IncrementalTransformerContextModel(nn.Module):
    ...
    def forward(self,xinput,has_mask=True,raw_out=False,mask=None):
        """
...
class TiedTransformerEncoder(nn.TransformerEncoder):
    """
...
if __name__ == '__main__':
     ...
     input_mask = open("json-test.json",'r')

     encoder, lm = load_transformer_model('TM', cpu=True)
     testset = Dataset('obj-pp_test.txt', parentencoding='TM',mask_stream=input_mask)

     #lm = LanguageModel(trainset,'LSTM',768,768,1,nheads=12,ffn_hidden=2048, dropout=0.1, tie_weights=True,device='cuda:1')
     for df in lm.predict(testset, batch_size=1, device='cpu',use_predefined_masks=False):
