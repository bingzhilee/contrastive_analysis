...
def run(task,trainset,validset,model_dir,devicelist):
    ...
    if task['context_model'] == 'MLM':
        lm = mlm.MLMLanguageModel(trainset,
    ...
    try:
        if task['context_model'] == 'MLM':
            lm.train_model(trainset,
                           ...
                           lr=LR,
                           modeldir=tmp_dir)
            nll = lm.validate(validset, task['batch_size'], task['bptt_chunk'], device=0)
...
class GridSearch:
    ...
    def search(self,trainfilename,validfilename,model_dir,device_list):
        ...
        trainset.save(model_dir) #Saves the dataset *encoding*(i.e. vocab) to file tokcodes

        print('Searching...')
        best_nll = 1000000000000
        best_task = None
